...
internal sealed class SerializationReferenceTracker(ReferenceHandler mode) : ISerializationReferenceTracker
{
    ...
    private Dictionary<object, uint> _lookup = new(ReferenceEqualityComparer<object>.Default);
    private uint _count;

    public ReferenceHandler ReferenceHandler => _mode;

    public bool TryRegister<T>(T value, out uint id) where T : class
    {
    ...
    private struct LookupScope(SerializationReferenceTracker tracker, Dictionary<object, uint> lookup) : IDisposable
    {
        public void Dispose()
        {
